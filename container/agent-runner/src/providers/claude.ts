...
import { writeMessageOut } from '../db/messages-out.js'; // PR #2440
import { getSessionRouting } from '../db/session-routing.js'; // PR #2440
...
function createPreCompactHook(assistantName?: string): HookCallback {
  return async (input) => {
    ...
    // PR #2440: notify the active channel before compaction
    try {
  ...
  query(input: QueryInput): AgentQuery {
    ...
    const sdkResult = sdkQuery({
      ...
      options: {
        ...
        pathToClaudeCodeExecutable: HOST_MODE ? 'claude' : '/pnpm/claude',
