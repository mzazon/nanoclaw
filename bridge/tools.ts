...
export function buildInstructions(sessionDir: string, assistantName?: string): string {
  ...
  if (process.env.NANOCLAW_BRIDGE_SCHEDULING !== '0') {
    sections.push('You can schedule, list, cancel, pause, resume, and update tasks using the scheduling tools.');
  }
  if (process.env.NANOCLAW_BRIDGE_NCL !== '0') {
    sections.push('Use the ncl tool to manage your agent group, list sessions, and query destinations.');
  }
  if (assistantName) {
