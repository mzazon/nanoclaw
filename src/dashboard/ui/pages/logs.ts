...
export function logsPage(): string {
  return layout(
    ...
    <div style="margin-bottom:12px;display:flex;gap:12px;align-items:center">
      <label style="font-size:12px;color:var(--text-secondary)">Filter:</label>
    ...
    function appendLine(text) {
      ...
      if (text.indexOf('ERROR') >= 0) div.style.color = 'var(--status-error)';
      else if (text.indexOf('WARN') >= 0) div.style.color = 'var(--status-warning)';
      else if (text.indexOf('DEBUG') >= 0) div.style.color = 'var(--text-muted)';
